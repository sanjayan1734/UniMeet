...
const eventDetails = ({route}) => {
  ...
  const fetchEvent = () => {
    ...
    axios({
      ...
      (res)=>{
        ...
        setEventName(res['data']['event'][0]['event_name'])
      },
      (err) => {
       console.log(err)
      }
  ...
  const registerEvent = () => {
    axios({
      method: 'post',
      maxBodyLength: Infinity,
      url: 'http://172.20.10.2:5000/events/registerEvent',
      params: {event_id: route.params['event_id'],
        ...
        (res)=>{
          ...
          if (res['data']['response'] == "event registered successfully") {
            if (!registrationStatus) {
              setRegistrationStatus(true)
              alert('Event registered')
            }
      ...
      <View style={styles.eventDetailsContainer}>
        <View style={styles.eventBox}>
          ...
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText}>{eventDetails['chief_guest']}</Text>
          </View>
